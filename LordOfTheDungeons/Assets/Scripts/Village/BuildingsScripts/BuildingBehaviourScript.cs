...
public class BuildingBehaviourScript : MonoBehaviour
{
    ...
    private void InitTavern()
    {
        ...
        if(Server.GetStartTimeBatiment(Server.GetCurrentVillage(),"TAVERN") > 60)
        {
            Server.SetStartTimeBatiment(Server.GetCurrentVillage(),"TAVERN");
        }
        ...
        for (int i = 0; i < Village.Tavern.Level + 1; i++)
        {
            if (i % 2 == 0)
            {
                ...
                d.transform.localScale = new Vector2(1f, 1f);
                Server.InitCaracter(Server.GetCurrentVillage());
                //creation perso
                Armor a = GameObject.Find("CharacterFactory").GetComponent<ArmorFactory>().CreateArmor(0, "Armure en cuir", 1, 3);
                Armor a = GameObject.Find("CharacterFactory").GetComponent<ArmorFactory>().CreateArmor(0, "Armure en cuir", 1, 3);
                Weapon w = GameObject.Find("CharacterFactory").GetComponent<WeaponFactory>().CreateWeapon(0, "Fronde", 1, 5);
                GameObject c = GameObject.Find("CharacterFactory").GetComponent<CharacterFactory>().CreateCharacter(0, "Fat�o Mavard", "Hobbit", 1, 10, 10, 4, 5,"Archer",w,a);
